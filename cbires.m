...
% check for SRM dataset existence
...
if (~isfield(handles, 'DistanceFunctions') && ~isfield(handles, 'numOfReturnedImages'))
    ...
    onto_numOfReturnedImgs = get(handles.popupmenu_NumOfReturnedImages, 'Value');
elseif (~isfield(handles, 'DistanceFunctions') || ~isfield(handles, 'numOfReturnedImages'))
    if (~isfield(handles, 'DistanceFunctions'))
        metric = get(handles.popupmenu_DistanceFunctions, 'Value');
        onto_numOfReturnedImgs = handles.numOfReturnedImages;
    else
...
if (metric == 1)
    L1(onto_numOfReturnedImgs, handles.queryImageFeature, handles.imageDataset.dataset);
elseif (metric == 2 || metric == 3 || metric == 4 || metric == 5 || metric == 6  || metric == 7 || metric == 8 || metric == 9 || metric == 10 || metric == 11)
    L2(onto_numOfReturnedImgs, handles.queryImageFeature, handles.imageDataset.dataset, metric);
else
    relativeDeviation(onto_numOfReturnedImgs, handles.queryImageFeature, handles.imageDataset.dataset);
end
...
% call svm function passing as parameters the onto_numOfReturnedImgs, queryImage and the dataset
[cmat] = svm(onto_numOfReturnedImgs, handles.imageDataset.dataset, handles.queryImageFeature, metric);
...
for k = 1:15
    ...
    [precision, recall] = svm(onto_numOfReturnedImgs, database, queryImageFeature, metric);
